...
\begin{enumerate}\itemsep0pt
  ...
  \item \textbf{Leiden}~\citep{leiden2019} (27\,ms, \textbf{2.8$\times$ speedup}):
    ...
    KeyCorridorS3R2 (95.1\%$\pm$9.8 vs.\ SI2E 67.5\%$\pm$27.9).
  \item \textbf{Infomap}~\citep{infomap2008} (24\,ms, \textbf{3.2$\times$ speedup}):
    best overall --- 95.3\%$\pm$4.6 on KeyCorridorS3R2, the highest of any method,
...
\textbf{(1) Community detection is the best overall approach.} On KeyCorridorS3R2,
Infomap achieves 95.3\%$\pm$4.6 and Leiden achieves 95.1\%$\pm$9.8 --- both
+28\,pp over SI2E (67.5\%$\pm$27.9) --- while training 3.2$\times$ and 2.8$\times$
faster respectively. Infomap has substantially lower variance (4.6 vs.\ 9.8).
...
\textbf{(3) k-means matches SI2E on RedBlueDoors.} FastSI2E k-means achieves
50.4\%$\pm$36.1 on RBD-6x6 --- comparable to SI2E (55.7\%$\pm$38.7) --- at
4$\times$ the training speed. The variance remains high (bimodal pattern).

\textbf{(4) Adaptive-$\beta$ helps with the original \si{} but not with k-means.}
\si{} + adaptive-$\beta$ raises RBD from 34\% to 53.4\% (2/5 $\to$ 4/5 seeds).
However, combining adaptive-$\beta$ with \fastsi{} k-means \emph{hurts} performance:
28.6\%$\pm$23.6 on KC-S3R2 and 30.0\%$\pm$36.5 on RBD --- both substantially
below either component alone. We discuss this interaction in \S\ref{sec:discussion}.

\textbf{(5) All FastSI2E variants beat SI2E on speed} with no accuracy loss on
DoorKey-8x8.

\textbf{(6) On RedBlueDoors-6x6, Infomap matches SI2E; Leiden underperforms.}
Infomap achieves 54.4\%$\pm$38.5, closely matching original SI2E (55.7\%$\pm$38.7),
while Leiden achieves only 27.5\%$\pm$38.9. The bimodal convergence pattern (seeds
either succeed $\sim$80\% or fail near 0\%) persists across all methods, indicating
it is an environment-level property rather than a clustering artefact.

\begin{table*}[t]
\centering
\caption{Success rate (\%) mean$\pm$std and FPS (3\,M frames, 3--5 seeds per condition).}
\label{tab:main}
\small
...
\midrule
\fastsi{} k-means & \textbf{100.0$\pm$0.0} & 67.3$\pm$40.3 & 50.4$\pm$36.1 & \textbf{1952} & \textbf{4.0$\times$} & 3--5 \\
\fastsi{} Leiden & \textbf{100.0$\pm$0.0} & 95.1$\pm$9.8 & 27.5$\pm$38.9 & 1364 & 2.8$\times$ & 3--5 \\
\textbf{\fastsi{} Infomap} & 99.5$\pm$0.7 & \textbf{95.3$\pm$4.6} & 54.4$\pm$38.5 & 1540 & 3.2$\times$ & 3--5 \\
\midrule
\si{} + adaptive-$\beta$ & --- & 46.5$\pm$21.4 & \textbf{53.4$\pm$27.4} & 488 & 1$\times$ & 5 \\
\fastsi{} + adaptive-$\beta$ & --- & 28.6$\pm$23.6 & 30.0$\pm$36.5 & $\sim$1900 & $\sim$3.9$\times$ & 5 \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[h]
\centering
\caption{Clustering method comparison (3M frames, 3--5 seeds per method).
  \dag{} = 3/5 seeds converge, 2/5 fail near 0\%.}
\label{tab:clustering}
\small
\begin{tabular}{lccccc}
\toprule
Method & DK-8x8 & KC-S3R2 & RBD-6x6 & FPS & Cost \\
\midrule
k-means  & 100.0$\pm$0.0 & 67.3$\pm$40.3\dag & 50.4$\pm$36.1 & 1952 & 1.5\,ms \\
Leiden   & 100.0$\pm$0.0 & 95.1$\pm$9.8 & 27.5$\pm$38.9 & 1364 & 27\,ms \\
Infomap  & 99.5$\pm$0.7  & \textbf{95.3$\pm$4.6} & \textbf{54.4$\pm$38.5} & 1540 & 24\,ms \\
\bottomrule
\end{tabular}
\end{table}
...
Both outperform k-means substantially on KC-S3R2 (95.3\%$\pm$4.6 and 95.1\%$\pm$9.8
respectively, +28\,pp each over SI2E). On RBD-6x6, Infomap (54.4\%$\pm$38.5) closely
matches original SI2E (55.7\%), while Leiden (27.5\%$\pm$38.9) underperforms ---
likely because the RBD state graph is sparser and modularity optimisation finds less
meaningful communities. Infomap is also marginally faster (24\,ms vs.\ 27\,ms).
For practical use we recommend Infomap unless the environment is known to have
simple state topology, in which case k-means' 4$\times$ speedup over SI2E may be
preferred.

\textbf{Why adaptive-$\beta$ hurts with k-means.}
\label{sec:discussion}
Adaptive-$\beta$ works by reducing the exploration coefficient as the agent's
recent success rate rises. With the original \si{}'s PartitionTree, clusters are
semantically coherent (built from the full graph hierarchy), so the adaptive
signal reliably tracks progress. With k-means, clusters are noisier and less
stable across updates: a successful episode may not register in the cluster
entropy signal, causing $\beta$ to oscillate and destabilise training. This
suggests adaptive-$\beta$ should be paired with a semantically reliable
clustering method (PartitionTree, or potentially Infomap/Leiden) rather than
a speed-optimised proxy. However, even Infomap+adaptive-$\beta$ on RBD-6x6
achieves only 25.2\%$\pm$35.7 --- below both Infomap alone (54.4\%) and
k-means+adaptive-$\beta$ (30.0\%) --- indicating that the bimodal pattern on
RBD is an environment-level property, not a clustering artefact.

\textbf{Bimodal convergence.}
Despite adaptive-$\beta$ with the original \si{}, KC-S3R2 and RBD still exhibit
high variance. This likely reflects an environment-level property (long planning
horizons, sparse terminal reward). Future work: entropy-conditioned bonus
annealing or a separate extrinsic value head.
...
\si{} on KeyCorridorS3R2 ($+$28\,pp each) while being 2.8--3.2$\times$ faster.
On RedBlueDoors-6x6 Infomap matches SI2E (54.4\% vs.\ 55.7\%) at 3.2$\times$
the speed.
