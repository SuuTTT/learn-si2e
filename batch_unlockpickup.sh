#!/usr/bin/env bash
# batch_unlockpickup.sh
# Run baseline / SE / VCSE (kNN) / SI2E (H₂) on MiniGrid-UnlockPickup-v0 × 3 seeds.
# Not in SI2E paper — medium difficulty: agent must pick up key, unlock door, reach goal.
# Grid: 11×6, max_steps=288, 3M frames.
#
# Usage: nohup ./batch_unlockpickup.sh > logs/unlockpickup.log 2>&1 &
...
RESULTS_BASE="/workspace/learn-si2e/results/unlockpickup"
...
run_si2e_method() {
    ...
    local model_name="up-${method}-s${seed}"
    ...
    echo "============================================================"
    echo "[RUN] UnlockPickup  method=${method}  seed=${seed}"
    echo "============================================================"
    cd "$SI2E_DIR"
...
run_vcse_original() {
    local seed="$1"
    local model_name="up-vcse-s${seed}"
    ...
    echo "============================================================"
    echo "[RUN] UnlockPickup  method=vcse(kNN)  seed=${seed}"
    echo "============================================================"
    cd "$VCSE_DIR"
